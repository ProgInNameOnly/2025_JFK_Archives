...
# Output text file for PDFs over 1 MB
output_text_file = "jfk_documents_over_1mb.txt"
...
try:
    ...
    with open(output_text_file, 'a', encoding='utf-8') as text_file:
        ...
        for filename in sorted(os.listdir(pdf_dir)):
            if filename.lower().endswith('.pdf') and os.path.isfile(os.path.join(pdf_dir, filename)):
                ...
                # Process only files over 1 MB (1,048,576 bytes)
                if file_size < 1048576:  # 1 MB
                    print(f"Skipping {filename} (size: {file_size / 1048576:.2f} MB) - under 1 MB")
                    continue
                ...
                try:
                    ...
                    # Step 2: If no text found, use OCR with higher timeout for larger files
                    if not text.strip():
                        print(f"No text found in {filename} with PyPDF2; attempting OCR...")
                        start_time = time.time()
                        timeout = 120  # 2 minutes timeout for larger files
                        images = None
                        try:
                        ...
                        if images:
                            for i, image in enumerate(images):
                                try:
                                    ocr_text = pytesseract.image_to_string(image, timeout=20)  # 20 seconds per page
                                    text += f"Page {i+1}:\n{ocr_text}\n\n"
                                except Exception as e:
    ...
    print(f"Completed. Processed {count} PDFs over 1 MB. Text dumped to {output_text_file}")
